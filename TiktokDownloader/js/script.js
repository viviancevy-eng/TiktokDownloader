document.addEventListener('DOMContentLoaded', () => {
  ...
  function showResult(result, downloadType) {
    ...
    if (resultTitleEl) resultTitleEl.textContent = 'Siap diunduh!';
    ...
    // Tidak auto-download; user memilih lewat tombol di hasil.
  }
  ...
  async function smartDownload(url, filename) {
    ...
    // Prefer fetch->blob to prevent navigating to third-party pages.
    // Note: This may fail due to CORS; then we fallback to direct link.
    try {
      ...
      return attemptAutoDownload(url, filename);
    }
  }
  ...
  if (downloadLink) {
    downloadLink.addEventListener('click', (e) => {
      e.preventDefault();
      smartDownload(downloadLink.href, downloadLink.download);
    });
  }

  if (downloadAudioLink) {
    downloadAudioLink.addEventListener('click', (e) => {
      if (downloadAudioLink.getAttribute('aria-disabled') === 'true') return;
      e.preventDefault();
      smartDownload(downloadAudioLink.href, downloadAudioLink.download);
    });
  }
